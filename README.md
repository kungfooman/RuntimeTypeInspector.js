...
- It converts the common TypeScript surface: functions with parameters/returns, interfaces and type aliases (`@typedef`), enums, generics (`@template`), classes including parameter properties, tuples, unions, rest parameters, default-value inference, TSX, `import x = require('...')` and `import('./x').T` type references.
- It is **not** a complete TypeScript compiler. `namespace` blocks are currently dropped (with an explanatory comment), and the snapshot suite in `test/ts2js.mjs` documents exactly what is currently covered.
