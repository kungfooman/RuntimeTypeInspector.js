...
function updateUser(id, updates) {
  // Valid usage: updates can be { name: "Alice" } or { email: "a@b.com" }
}
...
function createUser(payload) {
  // payload requires 'name', 'email', and 'role', but NOT 'id'
}
...
updateUser("1", {name: "Alice", email: "a@b.com", role: "admin"}); // ok: full update
displayPreview({name: "Alice", email: "a@b.com"}); // ok
createUser({name: "kungfooman", email: "lama12345@gmail.com", role: "admin"}); // ok
createUser({id: "7", name: "n", email: "e", role: "guest"}); // ok: id allowed, just not required
...
updateUser("1", {email: 1}); // warns: email must be string
displayPreview({name: "Alice"}); // warns: missing required email
displayPreview({name: "Alice", email: 1}); // warns: email must be string
createUser({ // warns: missing required role
  name: "kungfooman",
  email: "lama12345@gmail.com"
});
createUser({name: "n", email: "e", role: "superadmin"}); // warns: role not in union
